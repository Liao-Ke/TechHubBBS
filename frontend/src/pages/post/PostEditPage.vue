...
import type { Category, PostVisibility, PostVO } from '@/api/types'
...
const originalPost = ref<PostVO | null>(null)
...
// ---- Draft auto-save (keyed by postId) ----
const {
  currentDraftId,
  draftData,
  checkDraft,
...
watch(
  ...
  (data) => {
    draftData.value = { ...draftData.value, ...data, postId }
  },
  { deep: true },
)
...
async function handleSubmit() {
  ...
  try {
    ...
    const message = e instanceof Error ? e.message : '更新失败，草稿已保存'
...
<template>
  <div class="post-form-page">
    <header class="post-form-page__header">
      <h1 class="post-form-page__title">编辑帖子</h1>
      <p class="post-form-page__subtitle">修改你的帖子内容</p>
    </header>
